...
public class AppSettings
{
    ...
    public void NormalizeAndValidate(string workingDirectory, string? legacyBasePath = null)
    {
        var normalizedWorkingDirectory = NormalizeDirectoryPath(workingDirectory, null, nameof(workingDirectory));
        ...
        DataPath = NormalizeDirectoryPath(DataPath, normalizedWorkingDirectory, nameof(DataPath));
        ...
        Database.Path = NormalizeFilePath(Database.Path, DataPath, normalizedWorkingDirectory, "Database:Path", "adbclient.db");

        Logging ??= new AppSettingsLogging();
        Logging.File ??= new AppSettingsLoggingFile();
        Logging.File.Path = NormalizeFilePath(Logging.File.Path, DataPath, normalizedWorkingDirectory, "Logging:File:Path", "adbclient.log");
    ...
    private static string NormalizeFilePath(string? path, string dataPath, string workingDirectory,
                                            string settingName, string defaultFileName)
    {
        var useDefault = string.IsNullOrWhiteSpace(path);
        var configuredPath = useDefault ? defaultFileName : path!.Trim();
        ...
        try
        {
            ...
            // Released versions passed explicit overrides directly to SQLite and Serilog.
            // Preserve their working-directory-relative meaning; only defaults live in DataPath.
            return Path.GetFullPath(configuredPath, useDefault ? dataPath : workingDirectory);
        }
        catch (Exception ex) when (ex is ArgumentException or NotSupportedException or PathTooLongException)
        {
            throw new InvalidOperationException($"{settingName} is not a valid filesystem path.", ex);
